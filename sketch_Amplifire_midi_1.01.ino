...
// 5 foot switches are supported of the momentary close type and connecting Digital pins 2,3,4,5 and 6 to ground
...
byte FS5Presets[] = {50, 51}; // an array to hold the high and low volume settings
...
int FS5Index = 0; // a preset index fro FS5
...
void loop() {
  ...
  if (FS5.onPressed()){
    // Serial.println("FS3 pressed"); // test code
    // Serial.println(FS3Presets[FS3Index]); // test code
    progChange(FS5Presets[FS5Index]);
    FS5Index ++; // increment the preset index
    FS1Index = 0; // reset FS1 index
    FS2Index = 0; // reset FS2 index
    FS3Index = 0; // reset FS3 index
    FS4Index = 0; // reset FS5 index
    if (FS4Index > 1)  { FS4Index = 0; }
  }
